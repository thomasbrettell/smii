...
<script lang="ts">
  ...
  const scaleRatio = 500 / 50;

  const s = (val: number) => val / scaleRatio;
  ...
  $: eye1Translate = $mouthPoint1[1] <= $mouthPoint2[1] ? [10, -10] : [0, 0];
  $: eye2Translate = [-10, 10];
</script>
  ...
  <div class="svg-root">
    ...
    >
      <path
        ...
        fill="black"
        stroke="none"
        transform={`translate(${$mouthPoint1[0] + eye1Translate[0]}, ${$mouthPoint1[1] + eye1Translate[1]})`}
      />
      <path
        id="eye-2"
        ...
        fill="black"
        stroke="none"
        transform={`translate(${$mouthPoint4[0] + eye2Translate[0]}, ${$mouthPoint4[1] - eye2Translate[1]})`}
      />
